...
- `CaptainContext.emitReply` can no longer slip a `captain_reply` record past its turn's terminal record. The runtime cleared the active-turn identity only after the terminal `turn_finished` / `turn_aborted` record finished dispatching, while the emitReply gate checked only that identity, so a stashed context firing during terminal dispatch — an observer reacting to the terminal record, or any continuation scheduled in that window — passed the gate and appended a `captain_reply` after `turn_finished`, breaking the terminal-last record ordering observers and the tmux presenter assume (Captain prose painted after the turn had visibly closed and the ready prompt returned). The turn is now fenced immediately before its terminal record is enqueued, on the completed, the signal-aborted, and the runtime-failure paths alike: from that point `emitReply` rejects with the same turn-scope error as any other out-of-turn call and emits nothing, so the record stream never carries a `captain_reply` after its turn's terminal record, while a reply Promise obtained during the turn still drains ahead of the terminal record exactly as before — TMUX-092
