...
- The Kimi conformance target stays at `@agentclientprotocol/sdk` `0.23.0` with `@moonshot-ai/kimi-code` `0.27.0` although both have newer releases, because each move is a behavior change rather than a version bump. ACP `1.3.0` adds an `exports` map with no `./dist/*` subpath, so the adapter's deep import of the generated zod schemas is blocked with no public replacement at any subpath; the schemas it would replace are also deliberately lenient where `0.23.0`'s are strict — `zPromptResponse` silently drops an invalid `usage` instead of throwing — which would weaken the adapter's malformed-traffic detection rather than preserve it. Kimi Code `0.31.1` reshapes the `thinking` configuration option from a capability-gated binary into a per-model effort ladder, which the binary `off | on` control is specified against. Neither target forces the other: the ACP wire version is `1` in both SDK releases, and Kimi Code bundles its own ACP `0.23.0` server, so `kimi acp` `0.31.1` initializes cleanly against SDK `0.23.0`. Both moves are therefore left to a dedicated iteration that can amend the specifications they contradict — PKG-012, DR-011 rather than from a fixed `claude` + `codex` roster. The generated home config wires the built-in `fanout` Captain plus one player per installed adapter, taking the first two in the order `claude`, `codex`, `gemini`, `kimi`, `opencode`, and the one-line creation notice names those adapters so the roster is never a silent function of host state. With the Claude and Codex SDKs both installed the generated file is unchanged; a host with one provider gets a roster it can run instead of one it cannot. Generated roles carry `model` and `effort` only for the adapters this project pins (`claude-opus-4-8` and `gpt-5.5`, both at `xhigh`), because a portable `effort` is not valid for every adapter. The file stays user-owned: installing another provider later does not rewrite it. `--theme-diagnostics` no longer creates a config as a side effect, so reporting the terminal background never depends on an installed runtime — DR-012, IR-038, TMUX-010, TMUX-011, TMUX-061
