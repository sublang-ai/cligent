  ...
  try {
    runtime = await createTmuxPlayRuntime({
      ...
      players: PLAYER_ADAPTERS.map((adapter) => ({
        id: adapter,
        adapter,
        instruction: `You are player ${adapter}. Keep replies short and include required acceptance tokens exactly.`,
...
function adapterModel(adapter: PlayerAdapterName): { model?: string } {
