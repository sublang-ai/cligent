...
4. **Close residual package readiness verification.** Cover the remaining assertions in `package-103` through `package-105` and `package-201` across lifecycle acquisition, target history, isolated resolution, and verdict behavior after the already-checked Claude and Codex selected-runtime authority, metadata, and isolation cases.
...
45. **Cover the fourth tmux-play behavior slice.** Close every behavior branch without a proving assertion in the contiguous External Behavior slice from `tmux-play-31` through `tmux-play-62`, inclusive in section order.
46. **Cover the fifth tmux-play behavior slice.** Close every behavior branch without a proving assertion in the contiguous External Behavior slice from `tmux-play-68` through `tmux-play-50`, inclusive in section order.
47. **Cover the final tmux-play behavior slice.** Close every behavior branch without a proving assertion in the contiguous External Behavior slice from `tmux-play-46` through `tmux-play-189`, inclusive in section order.
