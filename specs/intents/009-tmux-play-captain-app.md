...
Out of scope: see [DR-004](../decisions/004-tmux-play-captain-architecture.md) §Out of Scope.
...
- [x] `src/app/tmux-play/players.ts` — player resolution and per-player `Cligent` construction.
- [x] `src/app/tmux-play/contract.ts` — `Captain` (with optional `init(session)` and `dispose()`), `BossTurn`, `CaptainSession`, `CaptainContext`, `CaptainTelemetry`, `PlayerHandle`, `RunStatus`, `PlayerRunResult`, `CaptainRunResult`. No per-call options until a consumer needs them.
...
- [x] `src/app/tmux-play/runtime.ts` — programmatic runtime API; turn loop, abort, `Captain.init(session)` and `dispose()`, `CaptainSession` (session abort, players, `emitStatus`, `emitTelemetry`), `CaptainContext` (turn abort, players, `callPlayer`, `callCaptain`), observer registration, shutdown drain order.
...
- [x] Tests: config + snapshot, player resolution, contract types, observer dispatch (multi-observer registration, status/telemetry drain, `turnId` nullable for session-scoped emissions), `init`/`dispose` lifecycle, shutdown drain order, fanout prompts and result collection, runtime causality.
...
- [x] README for `tmux-play`, config formats, snapshot mechanism, layout, and writing custom Captains.

## Tasks

1. Extract shared primitives (shell quoting, tmux invocation, log files, event formatting) under `src/app/shared/`.
2. Player resolution: `{ id, adapter, model, instruction }`, ID regex, reserved `captain` ID, duplicate adapters across players; test it.
3. Config loading for `.mjs`/`.js`/`.json` with discovery and `--config <path>`. Validate the Captain and player schemas; fail fast on unknown adapters, missing fields, unsupported extensions, invalid IDs, and non-serializable values.
...
5. Define the Captain extension contract types and runtime API in `src/app/tmux-play/`: `Captain` with optional `init(session)`/`dispose()`, `CaptainSession` (session abort, players, emit methods), `CaptainContext` (turn abort, players, run methods); wire the `@sublang/cligent/tmux-play` sub-export to expose these plus the observer-registration contract.
...
7. Runtime: turn serialization, persistent Captain and player `Cligent`s, `callPlayer`/`callCaptain` wrappers bound to `context.signal` with record emission, `CaptainSession` plumbing for `emitStatus`/`emitTelemetry`, `Captain.init(session)` (with observers attached first) and `dispose()` on shutdown. Implement the shutdown order: turn unwind → abort `session.signal` → drain accepted session emissions → `dispose()` → detach observers; reject post-abort emit calls.
8. Tmux presenter: route player records to player logs, Captain/Boss records to the Boss/Captain pane; consume `captain_status`; ignore `captain_telemetry` (it is for opt-in observers, not the Boss pane).
9. `fanout` Captain at `src/captains/fanout.ts`: factory, player prompts, concurrent player calls, bounded summary prompt, `callCaptain`.
10. Launcher: load config, run snapshot writer, build Boss-left/players-right tmux layout, set pane titles, attach.
11. Session CLI: read snapshot, dynamic-import `captain.from`, construct factory and players, register observers (tmux presenter plus any configured opt-in presenters), readline loop, observer dispatch, handle SIGINT/SIGTERM/EOF and `dispose()`.
...
- `tmux-play` (default discovery) launches a session with Boss/Captain on the left and player panes on the right.
...
- A non-tmux observer for one Boss turn asserts causality: `turn_started` first; per player `player_prompt` → `player_event`s → one `player_finished`; per `callCaptain()` `captain_prompt` → `captain_event`s → `captain_finished`; `turn_finished` last (or `turn_aborted` on abort).
...
- For the fanout Captain: player runs execute concurrently; Captain summary appears in the Boss pane after players settle; raw player events are not copied into the Boss pane.
