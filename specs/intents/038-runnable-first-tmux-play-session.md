...
Make the documented `npm install -g @sublang/cligent` plus `tmux-play` path reach a session that runs, by generating first-run defaults only from the adapters whose runtimes are installed and by reporting any unmet runtime with its install command before a tmux session exists, while the agent SDKs stay optional peers.
