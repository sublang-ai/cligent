...
Normalize OpenCode `ToolPart` lifecycle snapshots into one correlated `tool_use`/`tool_result` pair per `callID`, ending the duplicate empty `tool_use` stream and missing terminal results observed in dogfooding ([issue #33](https://github.com/sublang-ai/cligent/issues/33)).
