...
In progress.
Tasks 1 through 18 are done; tasks 19 through 36 remain.
...
Task 6 found the parser conflict that task 33 now carries, and the superseded-item cluster task 34 now carries, and completion waits on both.
Task 7 landed `package` with `package-1`, `package-5`, `package-8`, and `package-9` unverified, a gap it inherited and handed to task 32's table.
...
Task 11 landed `kimi` as the first adapter package whose every behavior a verification item reaches, so it hands task 32 no row, as `ndjson` already did at task 6.
...
The assertions no current check executes remain explicit in task 32, while the `package` locator in `specs/map.md` remains accurate without an edit.
...
The same review found [[package-16](../packages/package.md#package-16)] false of Claude's descriptor and runtime reader: SDK-domain compatibility versions are paired with a separately versioned, non-dependency `@anthropic-ai/claude-code` package name, so task 35 now carries the required design settlement ahead of handover.
...
- [ ] The per-item rename map built into the decision record in task 4 and reconciled in task 36, every classification and split judgment, and every open question reach a human diff review.
...
6. **Migrate `ndjson` and `release`.**
   ...
   The release behaviors carry no legacy verification at all, so add verification items only where a real check exists today and leave the residual coverage gap [[meta-33](../meta.md#meta-33)] to task 32 instead of inventing tests here.
...
15. **Settle the item-law rules every package split applies.**
    ...
    At that baseline, `claude-code-4` stated a missing `permissions` field maps to `permissionMode: 'default'` with no `canUseTool` callback, `gemini-12` and `codex-4` stated their own absent-policy rules, and task 32's table already recorded that `codex-224` asserts none of `codex-4`'s three unset controls.
    ...
    Each split carries its verification with it: every branch takes the assertion reaching it and the citation naming it [[meta-20](../meta.md#meta-20)], [[meta-33](../meta.md#meta-33)], and a branch no check reaches becomes a row of task 32's table in the same commit.
...
31. **Close Claude abort-listener cleanup.**
    ...
    Conform the artifact to the required per-run lifecycle without weakening effort rejection or invoking the SDK, and add an integration assertion that the rejected run leaves no caller listener behind; the code, check, and resulting task-32 handoff update land in this task's one commit.
    Leave this plan incomplete until it closes.

32. **Close or record the verification gaps.**
    ...
    | `engine`    | `engine-226`, whose cross-adapter upstream-effort-rejection flow no integration or system check runs | `engine-7`, whose exported `createEvent()`, `generateSessionId()`, and `isAgentEvent()` helpers no verification item cites; and `engine-29`, superseded by `engine-31` and cited by no verification item, which task 34 settles rather than task 32 |
...
33. **Settle the parser's unterminated-tail conflict.**
...
34. **Settle the superseded items.**
...
35. **Settle selected-vendor runtime identity.**
...
- `npm run typecheck`, `npm run lint`, `npm run test`, and `npm run build` stay green, as does every check task 32 adds or cites, including the smoke and acceptance suites `npm run test` excludes.
