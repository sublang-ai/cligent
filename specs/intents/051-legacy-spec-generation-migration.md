...
Task 39 retains each OpenCode permission request's denial correlation through an accepted reply or exact reply-time disappearance until matching native confirmation, consults the provider's pending registry to suppress both pre-confirmation repeats and post-confirmation stale replays while admitting a genuinely pending later lifecycle, gives registry lookup and reply one shared five-second permission-specific failure path, retains zero completed-response tombstones, releases failed, timed-out, aborted, and terminal request and wait state, and adds a high-volume sequence with 257 distinct completed requests that closes task 49's bounded-correlation handoff.
