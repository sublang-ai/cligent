...
Task 37 carries explicit zero and nonzero `maxTurns` through fresh and resumed v2 calls into the generated SDK's serialized `steps` request member, preserves omission when no limit is supplied, leaves legacy placement and `maxBudgetUsd` unchanged, and narrows task 49's remaining OpenCode option handoff.
...
49. **Close or record the verification gaps.**
    ...
    | `opencode` | `opencode-52`'s zero and omitted legacy limits, absent and empty model/cwd, and ignored-budget rows. | |
