...
In progress.
Tasks 1 through 36 are done; tasks 37 through 53 remain.
...
Task 36 retains valid same-session updates received during model, thinking, or mode configuration until after `init`, flushes them in arrival order through [[kimi-5](../packages/adapters/kimi.md#kimi-5)]'s ordinary normalization, keeps pre-prompt permission requests on [[kimi-27](../packages/adapters/kimi.md#kimi-27)]'s failure path, and adds the exact integration flow that closes task 49's init-first handoff.
