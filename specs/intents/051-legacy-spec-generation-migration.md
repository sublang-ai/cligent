...
Task 6 found the parser and superseded-item conflicts; tasks 50 and 51 have now closed both.
...
Task 13 landed `engine` with `engine-7` and its then-current supplementary-source item reaching none, deleted `test/adapters.md` and `test/engine.md`, moved the peer-SDK-floor clause to `package-201`, and restored `engine-219`, whose engine half task 12 had consumed because it lived in a body citation rather than a `Verifies:` line.
...
The same review found [[package-16](../packages/package.md#package-16)] false of Claude's descriptor and runtime reader: SDK-domain compatibility versions were paired with a separately versioned, non-dependency `@anthropic-ai/claude-code` package name; task 52 now settles that contradiction ahead of handover.
...
Task 26 made every live baseline `tmux-play` verification item from 101 through 150 lawful as one execution flow or explicit case matrix, while preserving `tmux-play-137`'s then-superseded wrapping passages for task 51.
...
Task 27 made every remaining live task-14 baseline `tmux-play` verification item lawful as one execution flow or explicit case matrix, while preserving the two then-retired left-click probes for task 51; [DR-019](../decisions/019-superseded-item-retirements.md) now records their retirement and [[tmux-play-168](../packages/tmux-play.md#tmux-play-168)] carries the joint verification.
...
Task 51 measured 37 marked package items — 35 wholly superseded and two partly live — plus the already-removed tmux-play wheel-clamp pair, records the owner's retirement decision and full per-item disposition in [DR-019](../decisions/019-superseded-item-retirements.md), removes every whole obsolete item, trims only the hand-written wrapping passages from [[tmux-play-46](../packages/tmux-play.md#tmux-play-46)] and [[tmux-play-137](../packages/tmux-play.md#tmux-play-137)], permanently reserves every retired released number, and retargets the released-concern map and historical references without restoring artifact behavior.
Task 52 records in [DR-013](../decisions/013-cligent-owned-runtime-compatibility.md) the owner's choice that the version-tied Claude Agent SDK is Claude's sole compatibility and readiness authority, removes the false package-like Claude Code target, derives selected-binary consistency only from SDK-owned metadata while leaving absent identity or version data unreported, keeps Codex's selected executable authoritative through an SDK-relative readiness-version lookup without removing released executable fallbacks, and adds repository and isolated-layout checks that prevent an unrelated top-level package from supplying either adapter's readiness identity or version.
...
26. **Apply item law to baseline `tmux-play` verification items 101 through 150.**
    ...
    Keep the real-`tmux` condition in the behavior-verifying owner derived from [[tmux-play-130](../packages/tmux-play.md#tmux-play-130)] and add that owner as a destination of the twelve other source rows carried at the time by `tmux-play-131` through `tmux-play-137`, [[tmux-play-139](../packages/tmux-play.md#tmux-play-139)], and [[tmux-play-146](../packages/tmux-play.md#tmux-play-146)] through [[tmux-play-149](../packages/tmux-play.md#tmux-play-149)].
