...
- Each commit leaves every citation resolving: the inbound citations across `specs/` and the item IDs quoted in `src/`, `scripts/`, and `.github/workflows/` are retargeted in the same commit that moves their package.
  ...
  Both directions expand every element a reader could follow before resolving it, a shorthand list such as `TMUX-038/039/049` otherwise passing both: its leading element is rewritten and resolves, while `/039` is neither legacy-form for the backward pattern nor a whole token the forward one can see.
...
15. **Bring the items into item law.**
    ...
    `claude-code-4` is the clearest instance, alone among the five adapter permission mappings in stating no `PermissionPolicy.mode` row, while `gemini-6` states that `mode` takes precedence over the per-capability fields without stating anywhere in its package what `'auto'` and `'bypass'` map to, so this task states that outcome too: the adapter maps `'auto'` to `permissionMode: 'auto'` and `'bypass'` to `permissionMode: 'bypassPermissions'` with `allowDangerouslySkipPermissions`, both ahead of the per-capability levels, and both already asserted — so a reimplementer reading only the package reaches neither, and the `ENG-021` binding the item lacks gains the phrase it needs.
