...
In progress.
Tasks 1 through 29 are done; tasks 30 through 53 remain.
...
Its artifact audit found [[opencode-44](../packages/adapters/opencode.md#opencode-44)] false because the maintained v2 wrapper omits `steps` from fresh and resumed prompt requests, so task 37 restores the released `maxTurns` mapping; found [[opencode-9](../packages/adapters/opencode.md#opencode-9)] false because SDK loading precedes caller-listener installation and managed readiness neither receives nor races the run-owned abort signal, so task 38 restores preflight and readiness cancellation; found [[opencode-35](../packages/adapters/opencode.md#opencode-35)] false because completed permission-response keys accumulate for the run's lifetime, so task 39 bounds and releases that correlation state; and found [[opencode-10](../packages/adapters/opencode.md#opencode-10)] false before readiness because that exit is normalized as a generic stream failure, so task 40 restores the released managed-crash outcome.
...
37. **Preserve OpenCode turn limits through the v2 prompt path.**
    Task 22 found [[opencode-44](../packages/adapters/opencode.md#opencode-44)] false on the maintained v2 SDK path: the adapter maps `AgentOptions.maxTurns` to the wrapper's `steps`, but that wrapper puts `steps` only on the legacy prompt body and omits it from fresh and resumed v2 prompt requests.
    Conform the v2 request to the released mapping without changing legacy placement or inventing a `maxBudgetUsd` control, preserving explicit zero and omitting the field only when `maxTurns` is absent.
    Add focused integration assertions for zero and nonzero limits on fresh and resumed v2 sessions plus omitted-limit preservation; the code, checks, and resulting task-49 handoff update land in this task's one commit.
    Leave this plan incomplete until it closes.
...
49. **Close or record the verification gaps.**
    ...
    | `engine` | `engine-203`'s Kimi exactly-one active-abort terminal, which task 35 also repairs; | |
    ...
    | `kimi` | `kimi-201`'s complete event order; empty text, non-text message, user-chunk, and no-result rows; interleaved distinct tools; remaining later-field merge and priority; kind and unknown names; parsed-object-string, raw-string, absent/null/empty, array, and other inputs; forced fallback, description, both terminal update forms, raw-output-null, joined-text, content-array and null outputs, duration, and remaining duplicate rows; exact payloads for all three plan forms; remaining permission payload and option-priority rows; every-stop resume and accounting fields; exact refusal error and recoverability; elapsed duration; exactly one terminal on ordinary, ACP-failure, and child-exit paths; and error-before-`done`. | |
    | `kimi` | `kimi-202`'s single application-configuration through runtime-created `Cligent` to Kimi ACP model and `thinking: 'on'` flow. | |
    | `kimi` | `kimi-203`'s queued-response and update draining, active-abort terminal-before-termination order and complete terminal-priority matrix that task 35 settles, active/configuration child cleanup and terminal counts, caller-listener and protocol-resource cleanup, and exactly one cleanup sequence, including final-grace idempotency that task 35 repairs. | |
    ...
    | `kimi` | `kimi-230`'s real default availability probe and its supported-version, nonzero, missing, timeout, below-floor, and non-mutation outcomes; arbitrary and coalesced framing beyond fixed midpoint splits; oversized decoded buffers; invalid JSON-RPC object, request, notification, response, error, id, and pending-id shapes; remaining invalid consumed payloads; handled traffic with valid unknown fields; handled pre-session and cross-session update traffic; per-failure termination; empty-resume `session/new` selection and the full resumed-run cwd/config/init/event lifecycle; unsupported negotiated protocol; omitted configuration calls; exact empty ACP capability advertisement; init cwd and model fallbacks; init-first ordering and pre-init update queue that task 36 repairs; exactly one spawn and cleanup sequence plus interrupted/error terminal counts and order; inherited environment and drained bounded stderr; one-text-block prompt; zero-valued limits; bare `KIMI_API_KEY` insufficiency; non-authentication ACP throws, asynchronous process errors, unexpected-signal and null-close rows, non-recoverable flags on the remaining process and operation failures, structured details priority, stderr inclusion and deduplication, and still-alive-after-`SIGKILL` outcome; complete terminal precedence that task 35 settles; and post-abort isolation. | |
    ...
    | `opencode` | `opencode-52`'s zero legacy limit, absent and empty model/cwd, omitted-limit and ignored-budget rows, plus fresh and resumed v2 zero and nonzero `maxTurns` delivery, with task 37 owning all v2 turn-limit rows. | |
    ...
    | `opencode` | `opencode-237`'s resumed-lineage identifier-less-child tolerance, task-part child ownership, resumed lifecycle addition, owned-parent `session.updated` adoption, owned-descendant deletion, malformed lifecycle preservation, remaining permission-name, request-ID, tool-use-ID, input, pattern, decision, status, response, denial-name, and denial-output aliases, defaults, and priorities; synchronous and non-`Error` reply failures; and bounded completed-response correlation that task 39 repairs. | |
