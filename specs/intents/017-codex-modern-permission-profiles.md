...
- The DR-005, [[engine-52](../packages/engine.md#engine-52)], [[codex-4](../packages/adapters/codex.md#codex-4)], [[tmux-play-153](../packages/tmux-play.md#tmux-play-153)], and [[tmux-play-11](../packages/tmux-play.md#tmux-play-11)] amendments (Task 1).
...
- [x] `specs/` — DR-005, engine-52, codex-4, tmux-play-153, and tmux-play-11 amended; the Codex permissions doc added as a reference; `map.md` indexes this IR.
...
- [x] `src/adapters/auto-mode.acceptance.test.ts` / `src/app/tmux-play/launcher.acceptance.test.ts` — codex-219 confirms Codex auto-mode write/delete behavior through the real SDK, and tmux-play-153 asserts the modern Codex knobs.

## Tasks

1. [x] **Spec** — amend DR-005 (Codex modern permission-profile model, determinism caveat, inherit deferred), engine-52 (orthogonal automation-posture / local-access-surface axis composition), codex-4 (the new mapping), tmux-play-153, and tmux-play-11; add the Codex permissions doc reference; index this IR in `map.md`.
...
4. [x] **Acceptance tests** — keep the codex-219 Codex auto-mode probe focused on real-SDK write/delete behavior, update the tmux-play-153 Codex seam probe to assert the modern knobs, confirm the `:workspace` profile still lets the temp-file write and delete proceed, and run `npm run test:acceptance`.
...
- After Task 4, `npm run test:acceptance` passes with the Codex auto-mode and tmux-play-153 probes asserting the modern knobs.
