...
All seven tasks are implemented: the spec foundation, the config weight vocabulary and `layout.initialVisible`, the launcher startup-from-visible-set refactor with a reusable player-area routine, the `setVisiblePlayers` contract and `player_view_changed` runtime emission, and the full-rebuild `LayoutObserver` registered in session mode.
...
- Spec items: new tmux-play-80..tmux-play-84, tmux-play-98, and tmux-play-179..tmux-play-185; amendments to tmux-play-64, tmux-play-28, tmux-play-16, tmux-play-17, tmux-play-20, tmux-play-21, tmux-play-90, tmux-play-11; `specs/map.md` package-summary refresh.
...
- A configurable replay count (the `200`-line tail is fixed by [DR-007](../decisions/007-tmux-play-dynamic-player-visibility.md)) or a YAML/Captain-API knob for it.
...
- [x] `specs/user/tmux-play.md` — amend tmux-play-64 (weight vocabulary), tmux-play-28 (visible-column shape from the visible set), tmux-play-90 (migration rewrite), tmux-play-11 (default canonical field), tmux-play-16 / tmux-play-17 (`setVisiblePlayers`), tmux-play-20 / tmux-play-21 (`player_view_changed` enumeration + `turnId`); add tmux-play-80 (`layout.initialVisible`), tmux-play-81 (`setVisiblePlayers` validation / rejection / scope-`turnId`), tmux-play-82 (`player_view_changed` record), tmux-play-98 (non-participating-observer rule), tmux-play-83 (`LayoutObserver` full-rebuild semantics), tmux-play-84 (hidden-pane scrollback limitation).
- [x] `specs/test/tmux-play.md` — add tmux-play-179 (weight vocabulary + alias + conflict + precedence), tmux-play-180 (`initialVisible` loader + shape derivation), tmux-play-181 (home-config migration rewrite + default config), tmux-play-182 (launcher startup panes for the initial visible set), tmux-play-183 (`setVisiblePlayers` accept/reject + `player_view_changed` emission), tmux-play-184 (non-participating observers ignore `player_view_changed`), tmux-play-185 (`LayoutObserver` real-tmux full rebuild + hidden-pane reconstruction).
...
- [x] `src/app/tmux-play/config.test.ts` — tmux-play-179 / tmux-play-180 / tmux-play-181 loader, migration, and default cases.
...
- [x] `src/app/tmux-play/launcher.test.ts` + `launcher.acceptance.test.ts` — tmux-play-182 startup-visibility geometry / ordering.
...
- [x] `src/__tests__` / `runtime` tests — tmux-play-183 / tmux-play-184 emission, validation, and non-participating-observer coverage.
...
- [x] `src/app/tmux-play/layout-observer` tests + `launcher.acceptance.test.ts` — tmux-play-185 real-tmux rebuild + hidden-pane reconstruction.
...
1. [x] **Spec — layout weights, migration, default config (docs-only).**
   Amend [[tmux-play-64](../packages/tmux-play.md#tmux-play-64)] so `layout.columnWeights` is a two-/three-element compatibility alias and the canonical fields are `layout.singlePlayerColumnWeights` (default `[1, 1]`) and `layout.multiPlayerColumnWeights` (default `[1, 1, 1]`), with strict positive-integer validation, the both-present-rejection rule, and the resolution precedence (explicit preset → matching alias → shape default).
   Amend [[tmux-play-28](../packages/tmux-play.md#tmux-play-28)] so visible-column shape derives from the current visible set rather than the configured roster size.
   Amend [[tmux-play-90](../packages/tmux-play.md#tmux-play-90)] for the home-config rewrite of `columnWeights` to the canonical field (two-element → single, three-element → multi), the both-present rejection, the no-rewrite of `--config` / cwd configs, and replace the stale "resolved `layout` defaults from tmux-play-64" wording with the dynamic-visibility layout fields.
   Amend [[tmux-play-11](../packages/tmux-play.md#tmux-play-11)] so the default home config uses `layout.multiPlayerColumnWeights: [1, 1, 1]`.
   Add [[tmux-play-179](../packages/tmux-play.md#tmux-play-179)] (weight vocabulary, alias, conflict, precedence) and [[tmux-play-181](../packages/tmux-play.md#tmux-play-181)] (migration rewrite + default config), and reconcile [[tmux-play-164](../packages/tmux-play.md#tmux-play-164)] to the new snapshot shape (resolved `singlePlayerColumnWeights` / `multiPlayerColumnWeights`) and validation rules.
   Update the `specs/map.md` TMUX package summary.

2. [x] **Spec — visibility API, record, observer (docs-only).**
   Add [[tmux-play-80](../packages/tmux-play.md#tmux-play-80)] (`layout.initialVisible`: optional non-empty duplicate-free subset of configured player IDs; default all players in `players` order; array order is startup pane order; weight shape derives from this set; no zero-player layout).
   Amend [[tmux-play-16](../packages/tmux-play.md#tmux-play-16)] and [[tmux-play-17](../packages/tmux-play.md#tmux-play-17)] to add `setVisiblePlayers(playerIds)`, and add [[tmux-play-81](../packages/tmux-play.md#tmux-play-81)] for its validation (non-empty duplicate-free subset), reject-before-any-record semantics, visible-set-unchanged-on-failure, and the session-vs-turn `turnId` convention.
   Amend [[tmux-play-20](../packages/tmux-play.md#tmux-play-20)] (enumerate `player_view_changed`) and [[tmux-play-21](../packages/tmux-play.md#tmux-play-21)] (its `turnId` carriage), add [[tmux-play-82](../packages/tmux-play.md#tmux-play-82)] for the `player_view_changed` record shape and one-per-accepted-call emission, and add [[tmux-play-98](../packages/tmux-play.md#tmux-play-98)] for the rule that non-participating observers (presenter, follow, timing, notification) ignore it.
   Add [[tmux-play-83](../packages/tmux-play.md#tmux-play-83)] (`LayoutObserver` full player-area rebuild: kill-non-Boss, recreate in order via the launcher split sequence, `tail -n 200 -f`, reapply titles / timer options / read-only input / mouse bindings / layout hooks / Boss focus; tracked list advances only on full success; repeated-list no-op; display-only best-effort failure that never aborts a turn; successful-reconciliation-before-later-player-output guarantee on the ordered dispatch path) and [[tmux-play-84](../packages/tmux-play.md#tmux-play-84)] (hidden-pane scrollback limitation).
   Add [[tmux-play-180](../packages/tmux-play.md#tmux-play-180)], [[tmux-play-182](../packages/tmux-play.md#tmux-play-182)], [[tmux-play-183](../packages/tmux-play.md#tmux-play-183)], [[tmux-play-184](../packages/tmux-play.md#tmux-play-184)], [[tmux-play-185](../packages/tmux-play.md#tmux-play-185)].
...
3. [x] **Config — weight vocabulary + migration + default.**
   ...
   Cover tmux-play-179 / tmux-play-181 in `config.test.ts`.
...
4. [x] **Config — `layout.initialVisible` + visible-set shape.**
   Add `layout.initialVisible` to `config.ts`: validate it is a non-empty, duplicate-free subset of configured player IDs; default to every configured player in `players` order; preserve array order; reject malformed entries per [[tmux-play-8](../packages/tmux-play.md#tmux-play-8)].
   ...
   Cover tmux-play-180 in `config.test.ts`.
...
5. [x] **Launcher — startup panes from the initial visible set.**
   ...
   Update `launcher.test.ts` and `launcher.acceptance.test.ts` for tmux-play-182 (startup geometry and ordering for a configured `initialVisible` subset, and the all-visible default).
...
6. [x] **Contract + runtime — `setVisiblePlayers` + `player_view_changed`.**
   ...
   Cover tmux-play-183 emission/validation and tmux-play-184 (existing presenter / follow / timing / notification observers ignore the record) in runtime/observer unit tests.
...
7. [x] **Presentation — `LayoutObserver` full rebuild.**
   ...
   Add `layout-observer` unit coverage and a real-tmux tmux-play-185 acceptance probe (rebuild kills non-Boss panes, recreates the requested set in order, replays the bounded log tail, reapplies pane config, no-ops on a repeated set, reconstructs a re-shown hidden player's pane from its log tail) in `launcher.acceptance.test.ts`.
