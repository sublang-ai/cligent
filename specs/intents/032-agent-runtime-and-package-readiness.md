...
Exact runtime targets, native permission defaults, validation tooling, clean package output, live acceptance, Codex trust isolation, and installed-tarball verification are complete.

The OpenCode tool-list claims below record the original delivery.
The current [[opencode-15](../packages/adapters/opencode.md#opencode-15)] contract supersedes them: OpenCode rejects either explicitly present tool-list option before SDK loading because its supported provider surface exposes no independent exact tool registry.

## Intent

Validate Cligent against exact release-candidate targets for all four coding agents and remove package or acceptance defects that could ship stale or unverified behavior.
Gemini CLI behavior, adapter-scoped effort behavior, and the SDK peer floors required by provider effort transports are outside this iteration; this iteration owns only their exact development and CI validation targets, plus the independent readiness defects listed below.
...
- [x] Packed-package and dependency-audit checks pass.
...
7. [x] **Verify and document the distributable.**
...
- Exact installed SDK and CLI versions equal the versions asserted by the
  ...
  surfaces, bundled Claude/Codex manifest metadata matches the exact targets,
  each CI-installed external CLI reports its version, and the OpenCode target
