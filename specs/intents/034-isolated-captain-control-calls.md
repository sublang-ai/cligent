...
The OpenCode-specific enforcement claims below record the original delivery and are superseded by the current [[opencode-15](../packages/adapters/opencode.md#opencode-15)] contract.
OpenCode 1.18.13 merges prompt `tools` into persistent permission rules instead of exposing an independent exact tool registry, so the current adapter rejects every explicitly present tool-list option before SDK loading.
The Captain control boundary remains fail-closed; OpenCode can no longer implement a tool-free Captain call.
