...
2. **Codex events → AgentEvent normalization**
   ...
   - File change events → `codex:file_change` extension event per [DR-002](../decisions/002-unified-event-stream-and-adapter-interface.md) extensibility
   - Errors → `error` event

3. **UPM → Codex controls mapping**
   - Map `PermissionPolicy` to Codex `ThreadOptions` per [DR-002](../decisions/002-unified-event-stream-and-adapter-interface.md):
