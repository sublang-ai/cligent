...
Move the Kimi conformance target to Agent Client Protocol SDK `1.3.0` and Kimi Code CLI `0.31.1` by giving the adapter its own wire schemas for the protocol subset it consumes, so validation no longer depends on an unpublished SDK build artifact.
