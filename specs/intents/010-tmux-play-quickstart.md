...
Implement [DR-004](../decisions/004-tmux-play-captain-architecture.md)'s Configuration as updated: YAML-only configs, cwd → `~/.config/tmux-play/config.yaml` discovery, and first-run auto-create.
...
Out of scope: see [DR-004](../decisions/004-tmux-play-captain-architecture.md)'s Out of Scope section.

## Deliverables

- [x] `package.json` — add `yaml` to `dependencies`.
- [x] `src/app/tmux-play/config.ts` — YAML loader, cwd→home discovery, first-run auto-create with stdout notice; remove the JS/JSON loader paths.
- [x] `src/app/tmux-play/index.ts` — drop `defineConfig`. The snapshot writer's `captain.from` rewriting stays unchanged.
- [x] Tests: loader round-trips YAML; cwd wins over home; auto-create writes the default; existing home file is preserved across runs; malformed YAML hard-errors with file context; local `captain.from` paths in cwd configs resolve correctly via the snapshot.
- [x] Docs: README pitch reflects the one-command flow; `docs/tmux-play.md` shows YAML, discovery order, and auto-create behavior.
