...
Make the Codex adapter start from global and non-hoisted installs of the packed package by resolving `@openai/codex/bin/codex.js` anchored inside the `@openai/codex-sdk` tree that owns the dependency, keeping the adapter's own resolution context as a fallback, and failing with an actionable diagnostic when no route resolves.
