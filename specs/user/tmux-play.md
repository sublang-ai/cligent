...
When that observer handles `turn_finished` with sink `desktop` on macOS, it shall also write one best-effort silent terminal notification escape to orchestrator stdout for the completed turn, terminated without raw BEL (`\x07`) and without launching a sound-cue command.
