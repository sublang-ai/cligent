...
Each reported peer-SDK command shall install into that tree when run as printed, with any path argument quoted so a shell keeps it one argument. A bare `npm install -g` follows the one global prefix npm itself resolves from its environment, its configuration files, and its own install location, and a bare project install lands in the nearest directory at or above the invoking one that holds a `package.json` or a `node_modules`. The command shall therefore omit `--prefix` only where the bare form provably lands in the resolved tree: a global tree confirmed as the prefix npm itself reports, or a project tree whose root is the nearest such directory from the invoking one. Everywhere else — a global tree npm does not report as its prefix, a project tree the command was not invoked from inside, or one whose bare install a nearer manifest or `node_modules` directory captures — the command shall name the tree with `--prefix`. Whether the install is global or project-local shall be determined from the running package's own tree — a project install root carries the manifest that defines it — and never from the working directory, which says nothing about where cligent was installed. Where no `npm install` invocation reaches the resolved tree, the launcher shall print no install command for that peer SDK; it shall name the package and the tree to place it in, and report that no command reaches it.
