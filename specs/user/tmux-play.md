...
_Status: retired and entirely non-normative. The paragraphs below record the original requirement in past tense for spec history; no clause in this item is in effect, and no `shall` text appears here. The active normative behavior for left-click on a tmux-play pane is owned by [TMUX-067](#tmux-067)._
_Summary of supersession: cancelling copy-mode on every pane in the session before focusing the clicked pane returned each scrolled pane to its live tail, which surfaced as the user-reported "previously focused pane jumps to the last line" defect; per [TMUX-067](#tmux-067), pane scroll position and any active copy-mode selection are preserved across focus changes, and the launcher installs only tmux's stock per-table `MouseDown1Pane` bindings._
...
The launcher shall install this behavior by binding `MouseDown1Pane` in the `root`, `copy-mode`, and `copy-mode-vi` key tables — tmux dispatches mouse events through the clicked pane's current key table, so all three tables shall carry the binding to fire regardless of whether the click target is in a mode at the time.
Each binding's body shall be an `if-shell -F #{==:#{session_name},<session>}` whose true branch (1) chains, for every pane index `i` in `0..paneCount-1`, an inner `if -F -t <session>:0.<i> '#{pane_in_mode}' 'send-keys -t <session>:0.<i> -X clear-selection'` clause separated by ` ; `, and (2) ends with the per-table stock tail — ` ; select-pane -t= ; send-keys -M` for `root` and ` ; select-pane` for `copy-mode` and `copy-mode-vi`.
The false branch shall be the per-table stock tail verbatim — `select-pane -t= ; send-keys -M` for `root` and `select-pane` for `copy-mode` and `copy-mode-vi` — so every other tmux session on the same server retains stock click-to-focus behavior; the `send-keys -M` tail in the `root` body shall not be omitted in either branch, since mouse-aware terminal applications (vim, less, htop) depend on it to receive forwarded clicks.
The `send-keys -X clear-selection` primitive shall be used in preference to the retired [TMUX-066](#tmux-066) `-X cancel` primitive, because `clear-selection` drops the selection without exiting copy-mode while `cancel` returns the pane to its live tail (which surfaced as the user-reported "previously focused pane jumps to the last line" defect under TMUX-066 and motivated the [TMUX-067](#tmux-067) supersession).
The per-pane `#{pane_in_mode}` gate is required because `send-keys -X clear-selection` targeted at a pane not in any mode raises tmux's "no key table" error; the gate also avoids spurious work on panes that have nothing to clear.
tmux key tables are server-global (see [TMUX-062](#tmux-062) / [TMUX-063](#tmux-063) / [TMUX-065](#tmux-065)), so the launcher's binding outlives the tmux-play session; the `if-shell` guard keeps the binding inert in every other session, and a server that previously held the retired [TMUX-066](#tmux-066) chain or the retired [TMUX-067](#tmux-067) stock-binding install is overwritten in place by the new binding.
Drag-select per [TMUX-062](#tmux-062) is unaffected: `MouseDown1Pane` fires first and clears any prior selection, then `MouseDrag1Pane` (tmux's stock binding) enters `copy-mode -M` on the dragged pane and begins a fresh selection there without touching other panes.
The right-click copy path of [TMUX-062](#tmux-062) is unaffected: right-clicking an active selection still runs `copy-pipe-and-cancel`, which exits copy-mode on the clicked pane and returns it to its live tail because that is the explicit user action of copying and leaving copy-mode.
The keyboard pane-switch bindings of [TMUX-063](#tmux-063) only call `select-pane -L` / `select-pane -R`, which do not enter or cancel copy-mode, so they preserve every pane's scroll position and selection.
