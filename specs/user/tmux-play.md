...
The launcher shall bind `MouseDown3Pane` in both key tables to `send-keys -X copy-pipe <system-clipboard-command>`, so right-clicking an active selection copies through tmux's normal copy path, pipes the selected text to the host system clipboard when a supported route is available, clears the active selection as visible copy-confirmation, and preserves the clicked pane's current copy-mode scroll position.
...
The body shall then be wrapped in a fenced code block and rendered through `renderMarkdown` per [TMUX-050](#tmux-050); the presenter shall strip `glow`'s trailing horizontal line padding from the successful rendered output before indenting it, without stripping leading whitespace, so the body does not reserve cells to the right of its visible content and the existing two-space continuation indentation remains unchanged.
...
After successful rendering, the presenter shall strip `glow`'s trailing horizontal line padding from every rendered line before any outer-margin trim or prefix grammar is applied.
This strip shall remove only right-side padding cells emitted by `glow`, including padding followed only by SGR resets; it shall not strip leading whitespace, so `glow`'s left margin and the presenter's existing indentation behavior remain unchanged.
After that, the presenter shall trim at most one leading and at most one trailing blank line from `glow`'s output — `glow`'s default outer paragraph margin — before the prefix grammar is applied.
...
The presenter shall then apply the [TMUX-038](#tmux-038) prefix grammar: the first nonblank line after the outer-margin trim carries the colored `<who>> ` prefix; any blank lines preceding it (e.g., the inner edge of a fenced-code frame) pass through unmodified; every nonblank continuation line carries the two-space hanging indent.
...
`glow` owns word-boundary wrapping, fenced-code preservation, table layout, and inline-style rendering inside the block; the presenter shall not reflow `glow`'s output.
