...
When `tmux-play` is invoked without `--session`, the CLI shall run launcher mode: resolve the config, verify the configured adapters' runtimes per [TMUX-089](#tmux-089), construct the tmux session, attach, and exit.
...
When `--theme-diagnostics` is supplied, the CLI shall run theme-diagnostics mode: load the same config the launcher would load, resolve the Catppuccin flavor per [TMUX-047](#tmux-047), print `selected: <flavor>` and `reason: <explicit|yaml|osc11|fallback>` to stdout, include the raw OSC 11 reply when one was received, and exit without checking for `tmux` or `glow`, creating a tmux session, or attaching. Theme-diagnostics mode shall additionally create no config: where discovery per [TMUX-009](#tmux-009) finds none and `--config` is not supplied, it shall resolve the flavor as it would for `theme: auto` rather than running the first-run creation of [TMUX-010](#tmux-010), so reporting the terminal's background never depends on an installed adapter runtime. `--theme-diagnostics` is launcher-mode only; when combined with `--session`, the CLI shall reject the invocation before dispatching session mode.
...
When `layout.columnWeights` and the canonical field for the same shape are both present, the loader shall reject the config per [TMUX-008](#tmux-008) rather than silently choosing one; a `layout.columnWeights` of one shape may coexist with the canonical field of the other shape.
...
The resolved weights govern column region widths per [TMUX-044](#tmux-044): each non-rightmost column `i` receives `floor(W * w_i / sum(w))` cells at window width `W`, and the rightmost column absorbs the remainder; the shipped defaults are 50/50 for the single-player layout and even thirds for the multi-player layout.
...
The `captain` object and each `players` entry may include a `permissions` object whose typed shape is [[engine-21](../packages/engine.md#engine-21)]'s `PermissionPolicy`: `mode` is `'auto' | 'bypass'`, `fileWrite` / `shellExecute` / `networkAccess` are each `'allow' | 'ask' | 'deny'`, and `writablePaths` is an optional array of workspace-relative path strings per [[engine-22](../packages/engine.md#engine-22)].
The loader shall validate and canonicalize `permissions.writablePaths` per [[engine-22](../packages/engine.md#engine-22)], then retain the accepted `permissions` value as that captain's or player's runtime-held call default per [DR-005](../decisions/005-per-adapter-permission-configuration.md); when complete `settings` is omitted, tmux-play shall supply that default at the `Cligent.run()` boundary per [TMUX-093](#tmux-093), where the adapter performs the SDK-knob mapping per [[engine-21](../packages/engine.md#engine-21)].
...
Per [DR-009](../decisions/009-adapter-scoped-effort-vocabularies.md), the `captain` object and each `players` entry may include `effort`, whose accepted values shall be scoped by that entry's `adapter`: `claude` accepts the portable set plus `ultracode`, `codex` accepts the portable set plus `ultra`, `gemini` and `opencode` accept only [[engine-20](../packages/engine.md#engine-20)]'s portable set, and `kimi` accepts only the provider-native binary values `off` and `on`.
The loader shall retain an accepted value as that captain's or player's runtime-held call default, supply it at the `Cligent.run()` boundary when complete `settings` is omitted per [TMUX-093](#tmux-093), and preserve the adapter/value correlation in the exported captain, player, and runtime configuration types.
...
Where at least one supported adapter's runtime is installed, when neither location holds a config and `--config` is not supplied, the launcher shall create the home location with a default config per [TMUX-011](#tmux-011), print a one-line notice on stdout naming the path and the installed adapters the roster was built from, and continue.
...
The default Captain and default players shall include `permissions: { mode: 'auto' }` per [TMUX-052](#tmux-052); for Codex this resolves to `on-request + auto_review` with the `:workspace` permission profile because [[codex-4](../packages/adapters/codex.md#codex-4)] maps `mode: 'auto'` with unset capability fields to `:workspace`.
...
`callPlayer` and `callCaptain` shall be turn-scoped: when either is called after its originating turn has ended, or after session shutdown per [TMUX-019](#tmux-019), the returned Promise shall reject before any record is emitted and the rejected call shall not run its player or Captain `Cligent` call.
The originating turn shall count as ended no later than the dispatch of its terminal record (`turn_finished` or `turn_aborted`), as for `emitReply` per [TMUX-092](#tmux-092): a call made during or after that dispatch — including from an observer handling the terminal record — shall reject, so a stashed `CaptainContext` cannot start a call whose records would follow its turn's terminal record.
Every turn-scoped `CaptainContext` surface — `callPlayer` and `callCaptain` here, `setVisiblePlayers` per [TMUX-081](#tmux-081), and `emitReply` per [TMUX-092](#tmux-092) — shall stop being admitted when the runtime resumes from the Captain's `handleBossTurn`, which is earlier than that bound: the Captain run is the turn's scope, so anything presented on those surfaces after that resumption — from an admitted call's own continuation, or from an observer handling a record that drains afterwards — shall reject before any record is emitted, and a rejected `callPlayer` / `callCaptain` shall not run its `Cligent` call. One boundary governs all four, so a Captain need not reason about which surface outlives its turn. Closing admission there is what bounds the join of [TMUX-022](#tmux-022) to a set that cannot grow while it is awaited. The boundary is the runtime's resumption rather than `handleBossTurn`'s settlement because a continuation the Captain scheduled on that promise runs before the runtime regains control; such work is still admitted, and settlement is therefore not a boundary any implementation could enforce.
Closing admission shall not end the turn: through the join and drain of [TMUX-022](#tmux-022), and until the pre-terminal fence, the turn shall remain abortable per the preceding paragraph, so ESC, an external signal, and disposal still unwind the calls it admitted, and session-scoped emissions shall keep carrying its id per [TMUX-021](#tmux-021).
...
A `CaptainContext` call shall additionally be turn-scoped: when it is made after its originating turn has ended — counting the turn as ended no later than the dispatch of its terminal record (`turn_finished` or `turn_aborted`), as for `emitReply` per [TMUX-092](#tmux-092), and including a call from an observer handling that terminal record — the returned Promise shall reject before any record is emitted and the visible set shall remain unchanged, so the record stream shall never carry a `player_view_changed` stamped with a turn ID after that turn's terminal record.
Admission for this surface closes at [TMUX-016](#tmux-016)'s earlier boundary, when the runtime resumes from `handleBossTurn`: the terminal-record bound above is the outer limit, not the moment the surface stops being admitted.
...
When `emitReply` is called after its originating turn has ended, or after session shutdown per [TMUX-019](#tmux-019), the returned Promise shall reject and no `captain_reply` record shall be emitted. Admission closes at [TMUX-016](#tmux-016)'s earlier boundary, when the runtime resumes from `handleBossTurn`, so "active" above means the Captain run itself rather than everything preceding the terminal record.
...
This ordering shall not depend on how the Captain consumed a call: a `callPlayer` / `callCaptain` call the Captain started without awaiting shall emit its whole sequence before the turn's terminal record, on the failure path as on the completed one. The turn shall join every call it admitted before dispatching that record, so no record carrying a turn's ID follows that turn's terminal record. A call that outlives its turn's `handleBossTurn` therefore keeps the turn open until it settles, bounded by the turn's abort per [TMUX-016](#tmux-016).
...
Each reported peer-SDK command shall install into that tree when run as printed, with any path argument quoted so a shell keeps it one argument. Every peer-SDK command shall name the tree with `--prefix`: where a bare `npm install [-g]` lands is a property of the shell the command is pasted into — the global prefix npm resolves there, which npm rewrites in the environment of every lifecycle child, or the nearest enclosing project of the directory it is pasted in, which need not be the launching one — and no observation from the launching process can witness that shell, while npm's command line outranks both its environment and its project discovery, so the pinned form lands in the named tree in every context. Every peer-SDK command shall also pin its install scope on the command line, because npm's global mode is the disjunction of its `global` and `location` configurations and either is settable by the paste-time environment, diverting a prefix-pinned project install into `<prefix>/lib/node_modules`: a project command shall set both to their non-global values, and a global command shall assert global mode, which alone wins the disjunction. Whether the install is global or project-local shall be determined from the running package's own tree — a project install root carries the manifest that defines it — and never from the working directory, which says nothing about where cligent was installed. Where no `npm install` invocation reaches the resolved tree, the launcher shall print no install command for that peer SDK; it shall name the package and the tree to place it in, and report that no command reaches it.
Vendor credentials are outside this gate: they are not installed by a command, they surface as the provider's own run-time error, and per [TMUX-051](#tmux-051)'s precedent the launcher gates only on what an install command repairs.
The gate shall run after the `tmux` and `glow` checks of [TMUX-051](#tmux-051), which need no config, and shall report every unmet adapter in one error rather than the first.
...
The toast shall be a status-line `display-message` (not a floating `display-popup`) so it inherits the session's `message-style` (`fg=<base>,bg=<peach>` per [TMUX-047](#tmux-047)) and renders `Copied!` in the resolved flavor's base-on-peach styling — the same status-message band tmux uses for its own messages (dark text on Mocha's light peach, light text on Latte's vivid peach).
...
The flavor shall resolve in this priority: (1) explicit `themeFlavor` on the programmatic `launchTmuxPlay` option, when present and one of `'mocha' | 'latte'`; (2) the YAML config's `theme` field per [TMUX-006](#tmux-006), when present and one of `'mocha' | 'latte'`; (3) after those concrete overrides are exhausted, an OSC 11 terminal-background query when auto-detection is active and the launcher is going to attach, including a managed launch prepared for later native attachment per [TMUX-094](#tmux-094), or when [TMUX-061](#tmux-061) diagnostics mode is explicitly requested; (4) default Mocha.

The OSC 11 query shall write `OSC 11 ; ? BEL` (`\x1b]11;?\x07`) to the controlling terminal, read for a bounded short timeout, accept either BEL or ST termination, parse `rgb:RR/GG/BB` and `rgb:RRRR/GGGG/BBBB` replies, compute relative luminance as `0.2126 * R + 0.7152 * G + 0.0722 * B` over normalized channel values, and select Latte for luminance `>= 0.5`, otherwise Mocha. Failure to open the controlling terminal, failure to receive a parseable reply, a public launch with no eventual native attachment (`attach: false`) or non-TTY stdin/stdout, or timeout shall select Mocha with reason `fallback`. The launcher shall write the resolved (concrete) flavor into the session work-dir snapshot per [TMUX-034](#tmux-034) so the session subprocess uses the same flavor for pane-content SGR colors per [TMUX-038](#tmux-038) without re-running detection.
...
Within a single tmux-play session, each player's `Cligent` instance shall be created once and reused across every Boss turn. Per [[engine-5](../packages/engine.md#engine-5)], the engine shall auto-inject `resume` on subsequent runs when the underlying adapter emits a `resumeToken`, so player responses on later turns may build on prior context for adapters that support session continuity.
...
At call admission, tmux-play shall resolve the effective session selection exactly once from the explicit token, forced-fresh selection, or stored automatic token per [TMUX-041](#tmux-041) and [TMUX-088](#tmux-088); reset preflight and the eventual `Cligent.run()` invocation shall use that same detached selection.
...
Where a concrete Gemini effort has no model alias from [[gemini-11](../packages/adapters/gemini.md#gemini-11)], or a concrete OpenCode effort has no variant from [[opencode-12](../packages/adapters/opencode.md#opencode-12)], the call shall reject rather than silently ignore the effort.
Where a resumed Claude call selects a provider-default model, the call shall reject because [[claude-code-6](../packages/adapters/claude-code.md#claude-code-6)] exposes only omission, which restores the prior transcript model; where it supplies a concrete model with provider-default effort, tmux-play shall omit effort per [[claude-code-8](../packages/adapters/claude-code.md#claude-code-8)] so Claude uses that model's default effort.
Where a resumed OpenCode call selects a provider-default model, the call shall reject because OpenCode persists the prior session model and variant and [[opencode-12](../packages/adapters/opencode.md#opencode-12)] exposes no model reset; where it supplies a concrete model with provider-default effort, tmux-play shall omit the variant per [[opencode-14](../packages/adapters/opencode.md#opencode-14)] so OpenCode resets that model to its default effort.
Where a resumed OpenCode call's complete settings omit permissions, tmux-play shall clear its prior Cligent-owned session permission ruleset through [[opencode-7](../packages/adapters/opencode.md#opencode-7)]'s session permission surface before prompting rather than preserve a policy from an earlier call.
Where a resumed Kimi call selects provider-default model or effort or omits complete permissions, the call shall reject because [[kimi-4](../packages/adapters/kimi.md#kimi-4)], [[kimi-7](../packages/adapters/kimi.md#kimi-7)], and [[kimi-9](../packages/adapters/kimi.md#kimi-9)] expose no operation that restores a resumed session's provider default model, effort, or permission mode; a fresh Kimi call may use provider-default and no permissions, while a resumed Kimi call shall use concrete model and effort values plus an adapter-enforceable complete permission policy.
...
When the layout observer rebuilds the player area per [TMUX-083](#tmux-083), it shall reassign the same keys to the recreated panes, so every pane carries its key for the session's whole life.
Pane-addressed operations — pane-width soft-wrap, the per-pane border timers per [TMUX-054](#tmux-054), and copy-mode live-follow per [TMUX-069](#tmux-069) — shall resolve their target pane through that logical key and tmux's stable pane id, and machine-readable pane queries shall separate fields with a character no logical key can contain, so no operational lookup parses the displayed pane title of [TMUX-048](#tmux-048).
