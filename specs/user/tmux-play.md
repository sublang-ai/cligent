...
The false branch shall reproduce the per-table tmux stock binding for `Escape` so other tmux sessions on the same server retain stock behavior: `send-keys Escape` for the `root` table (forward the original key verbatim to the active pane) and `send-keys -X cancel` for `copy-mode` and `copy-mode-vi` (the stock copy-mode key that exits copy-mode).
...
The readline ESC handler is the single point of ESC interpretation regardless of which pane in the launched session the Boss pressed the key in: ESC pressed in a player pane (whose `pane-input-off=1` would otherwise swallow the byte) or in any pane scrolled back into copy-mode (where the stock copy-mode `Escape` binding would otherwise cancel copy-mode before reaching the root table) is forwarded to pane index 0 by [TMUX-070](#tmux-070), where this item's readline handler runs unchanged.
