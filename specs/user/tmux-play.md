...
`glow`'s code-block rendering owns the body's styling and leaves long code lines unwrapped by design; the prior `overlay0` `#6c7086` dim SGR around the body is no longer applied because `glow`'s styling supersedes it. Leading and trailing blank lines in `glow`'s rendered body output shall be trimmed before the two-space indent is applied, matching [TMUX-050](#tmux-050)'s text-body edge trim so the body does not stack `glow`'s paragraph margins under the header line. When `renderMarkdown` raises a mid-session failure (rare given the [TMUX-051](#tmux-051) launcher gate), the presenter shall emit the raw body text under the same two-space continuation indent rather than crash the session.
...
After rendering, leading and trailing blank lines in `glow`'s output (its default paragraph-margin padding) shall be trimmed before the prefix grammar is applied; internal blank lines between paragraphs are preserved. The presenter shall then apply the [TMUX-038](#tmux-038) prefix grammar: the first content line carries the colored `<who>> ` prefix; every nonblank continuation line carries the two-space hanging indent; preserved internal blank lines stay blank. An all-blank rendered block (after edge trim) shall emit no bytes to the writer, so empty content never surfaces as a stranded `<who>> ` line or a parade of padding blanks between consecutive turns.
