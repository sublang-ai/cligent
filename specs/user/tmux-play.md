...
Ownership shall precede mutation: the launcher shall not truncate, overwrite, or create artifacts a same-name session could share — its per-player logs, its config snapshot, or the session marker file the launcher writes into the work directory to tag it as session-owned — before acquiring the session name, so a launch rejected for a collision leaves the existing session's artifact bytes unchanged.
