...
Kimi Code `0.31.1` admits either a prior interactive OAuth `kimi login` or a configured default model resolving to a provider with non-OAuth credentials; `MOONSHOT_API_KEY` alone satisfies neither, because it configures no default model alias.
The acceptance harness exercises the OAuth route, so its credential probe reports on that route alone and self-skips when that credential is spent.
