...
Kimi Code `0.27.0` requires a prior interactive OAuth `kimi login` for ACP session creation; `MOONSHOT_API_KEY` or an API-key provider configuration alone does not satisfy that gate. The Kimi leg shall therefore run only when `CLIGENT_KIMI_ACCEPTANCE_HOME` names an absolute, dedicated source home containing `config.toml` and the OAuth `credentials/` directory. The harness shall dereference and copy only those entries into a temporary `KIMI_CODE_HOME`, harden the copied config, credential files, and directories to owner-only permissions, restore the caller's environment, and remove the temporary home. It shall never implicitly use or mutate the ordinary user home, and shall self-skip the Kimi live leg when the explicit source home is absent, including under `CI`; locally it shall retain the ordinary per-adapter skip for a missing CLI or invalid fixture. The source fixture is disposable because an OAuth refresh against the clone may leave its prior token stale.
