...
Given an empty home and cwd, when launching `tmux-play` without `--config`, the home YAML shall be created with the default `fanout` Captain plus `claude` and `codex` roles with identity instructions, the default Captain and `claude` role shall use `model: claude-opus-4-7` with `reasoningEffort: xhigh`, the default `codex` role shall use `model: gpt-5.5` with `reasoningEffort: xhigh`, and both default roles and the default Captain shall carry `permissions: { mode: 'auto' }`. A one-line notice naming the path shall be printed to stdout, and a second invocation shall not overwrite the file.
...
Given a real tmux server with one Captain pane and at least two configured role panes, when a `TimingObserver` receives synthetic `turn_started`, `role_prompt`, `role_finished`, `captain_prompt`, `captain_finished`, and `turn_finished` records with controlled timestamps, each pane-scoped timer option shall carry the expected cumulative duration for that pane, and the session-scoped total option shall carry the expected turn duration. Given a role or Captain run that is still open when the observer refreshes with a supplied `now`, the displayed duration shall include `now - <open-start>.timestamp`, use the running glyph `⏳`, and use the bright role/Captain accent; after the matching finished record, it shall freeze with glyph `⌛` and `overlay1`. Given an open Boss turn, the status-total timer shall include `now - turn_started.timestamp`, render on `status-right` with glyph `⏰` and `mauve`; after `turn_finished`, it shall freeze with `overlay1`. The real tmux session shall report navigation hints on `status-left`, the total timer on `status-right`, and a `pane-border-format` that references the pane timer slot without removing `#{pane_title}`. The probe shall assert tmux state via `show-options`, `show-options -p`, and `display-message`, and shall tolerate one cell of visual border-alignment variance for emoji glyph width. It shall run under `*.acceptance.test.ts`, shall not require adapter API keys, and shall self-skip when either `tmux -V` or `glow -v` fails.
