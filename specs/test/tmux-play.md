...
The acceptance probe shall additionally pin the observable consequence required by [TMUX-068](../user/tmux-play.md#tmux-068) (not only the binding string): on a real tmux server, given pane A in the launched session is placed in copy-mode with an active selection and pane B in the launched session is placed in copy-mode without a selection (a scrolled-back pane), when the binding's true branch is executed against that session, `#{selection_present}` on pane A shall be `0` (selection cleared), `#{pane_in_mode}` on pane A shall remain `1` (still in copy-mode at the same scroll position), `#{pane_in_mode}` on pane B shall remain `1` (still scrolled back, scroll preserved), and a third pane C that was never in copy-mode shall remain not in any mode. Pinning the behavioral outcome — selection cleared, scroll preserved — directly catches the regression that [TTMUX-067](#ttmux-067) missed by asserting only the binding strings.
