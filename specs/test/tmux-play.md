...
Given the launcher constructing a tmux-play session, the tmux command stream shall include `set-option -t <session> mouse on`, shall include `bind-key -T copy-mode MouseDragEnd1Pane send-keys -X stop-selection`, `bind-key -T copy-mode-vi MouseDragEnd1Pane send-keys -X stop-selection`, and a `bind-key -T copy-mode MouseDown3Pane` plus a `bind-key -T copy-mode-vi MouseDown3Pane` binding whose bound command is a single `if-shell -F '#{selection_present}'` with a true branch `display-message Copied! ; send-keys -X copy-pipe '<system-clipboard-command>'` and a false branch `send-keys -X copy-pipe '<system-clipboard-command>'`, shall not include a `set-clipboard` option write, and shall not include any `WheelDownPane` binding.
...
The acceptance probe shall run under `*.acceptance.test.ts`, shall not require adapter API keys, and shall self-skip when either `tmux -V`, `glow -v`, or an attached-client key driver is unavailable.
