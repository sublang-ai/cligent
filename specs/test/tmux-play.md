...
_Superseded by [TTMUX-067](#ttmux-067): [TMUX-066](../user/tmux-play.md#tmux-066) is retired in favor of [TMUX-067](../user/tmux-play.md#tmux-067); the assertions below now describe behavior the launcher must NOT install. The original criteria are retained for spec history; the new positive verification lives in [TTMUX-067](#ttmux-067)._
...
`<trueBranch>` shall chain, for every pane index `i` in `0..paneCount-1`, an `if -F -t <session>:0.<i> '#{pane_in_mode}' 'send-keys -t <session>:0.<i> -X cancel'` clause separated by ` ; `, followed by the per-table tail: ` ; select-pane -t= ; send-keys -M` for the `root` table and ` ; select-pane -t=` for `copy-mode` and `copy-mode-vi`.
...
Given a real tmux server, when `launchTmuxPlay({ attach: false })` returns, `tmux list-keys -T root MouseDown1Pane` shall report a binding whose body contains `if-shell`, `session_name`, the launched session name, `pane_in_mode`, `send-keys` with `-X cancel`, `select-pane -t=`, and `send-keys -M`; and `tmux list-keys -T copy-mode MouseDown1Pane` together with `tmux list-keys -T copy-mode-vi MouseDown1Pane` shall each report a binding whose body contains `if-shell`, `session_name`, the launched session name, `pane_in_mode`, `send-keys` with `-X cancel`, and `select-pane`.
...
Given the launcher constructing a tmux-play session whose `sessionName` is `<session>`, the tmux command stream shall include exactly one `bind-key -T root MouseDown1Pane 'select-pane -t= ; send-keys -M'` invocation and exactly one `bind-key -T <table> MouseDown1Pane 'select-pane'` invocation for each `<table>` in `copy-mode`, `copy-mode-vi` — the stock per-table tmux defaults verbatim — and shall not include any `MouseDown1Pane` argv that references `if-shell`, the launched session name, `#{pane_in_mode}`, or `send-keys -X cancel`.
The `mouse` option shall still be set to `on`, and the `MouseDragEnd1Pane` stop-selection and `MouseDown3Pane` system-clipboard right-click-copy bindings of [TTMUX-062](#ttmux-062) shall still be installed unchanged, so this item only retires the [TTMUX-066](#ttmux-066) cancel-on-every-pane logic and leaves every other [TMUX-062](../user/tmux-play.md#tmux-062) mouse binding intact.
Given a real tmux server, when `launchTmuxPlay({ attach: false })` returns, `tmux list-keys -T root MouseDown1Pane`, `tmux list-keys -T copy-mode MouseDown1Pane`, and `tmux list-keys -T copy-mode-vi MouseDown1Pane` shall each report a binding whose body matches the corresponding stock per-table tail — `select-pane -t= ; send-keys -M` for `root` and `select-pane` for `copy-mode` and `copy-mode-vi` — with no reference to the launched session name, `pane_in_mode`, or `-X cancel`, even on a tmux server that previously held the retired [TTMUX-066](#ttmux-066) `if-shell` binding from an earlier launcher run.
This pins the observable consequence required by [TMUX-067](../user/tmux-play.md#tmux-067): under the launcher-installed stock bindings a left-click only changes pane focus and never cancels copy-mode on any pane, so a pane that was scrolled up or holds an active selection preserves both across focus changes.
The acceptance probe shall run under `*.acceptance.test.ts`, shall not require adapter API keys, and shall self-skip when either `tmux -V` or `glow -v` fails.
