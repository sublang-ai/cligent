...
The test suite shall additionally fail unless the peer-SDK command names the resolved tree with `--prefix` wherever a bare `npm install` would not select it: a global tree that is not the prefix npm itself reports — including a tree matching npm's built-in derivation while the environment redirects the effective prefix elsewhere — a project tree the command was not invoked from, and a project tree invoked from a subdirectory whose bare install a nearer `package.json` or `node_modules` directory captures. A `--prefix` path a shell would split shall be printed quoted and still target the reported tree. A project install shall stay a project install when invoked from outside its own project — classified by the manifest at its install root rather than by the working directory — and a resolved tree that no `npm install` invocation reaches shall carry no peer-SDK install command, naming instead the package and the tree to place it in.
