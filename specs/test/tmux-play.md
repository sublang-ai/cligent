...
Given a `NotificationObserver` configured with `turn_finished: desktop`, when it receives one `turn_finished` record on macOS, it shall launch exactly one detached best-effort `osascript` notification command, shall write exactly one terminal BEL (`\x07`) to orchestrator stdout, and shall not launch an `afplay` sound command.
Given a `NotificationObserver` configured with `player_finished: desktop` or `turn_aborted: desktop`, when it receives the matching record on macOS, it shall launch exactly one detached best-effort `osascript` notification command and shall write no terminal BEL (`\x07`) or terminal notification escape bytes to orchestrator stdout.
Given a `NotificationObserver` configured with `turn_finished: desktop`, when it receives one `turn_finished` record on Linux, it shall launch exactly one detached best-effort `notify-send` OS notification command and shall write no terminal BEL (`\x07`) or terminal notification escape; on other platforms it shall launch no command and write no terminal BEL.
...
Given a text-body prose block rendered in a 40-cell pane by a real `glow` binary, at least one two-space-indented continuation row shall be wider than the old `40 - prefixWidth` text budget while no visible row exceeds 40 cells. This pins the user-reported "empty right side of every pane" defect that remained after the trailing-padding strip: the output must use the continuation width, not the first-line speaker-prefix width, for non-first prose rows.
