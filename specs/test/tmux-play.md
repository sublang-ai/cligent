...
The test suite shall additionally fail unless the peer-SDK command names the resolved tree with `--prefix` wherever a bare `npm install` would not select it: a global prefix that is not npm's own default root, and a project tree the command was not invoked from. A project install shall stay a project install when invoked from outside its own project — classified by the manifest at its install root rather than by the working directory — and a resolved tree that no `npm install` invocation reaches shall be reported as such instead of carrying a command.
