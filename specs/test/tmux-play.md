...
The `mouse` option shall still be set to `on`, and the `MouseDragEnd1Pane` stop-selection and `MouseDown3Pane` system-clipboard right-click-copy bindings of [TTMUX-062](#ttmux-062) shall still be installed unchanged.
...
Given the live-follow `RecordObserver` constructed for a tmux-play session with destination panes resolved per [TMUX-040](../user/tmux-play.md#tmux-040), when it observes a record that drives a presenter pane write — a record that flushes or writes a block per [TMUX-050](../user/tmux-play.md#tmux-050), e.g. `captain_finished` / `player_finished`, `captain_status`, `turn_aborted`, `runtime_error`, `player_prompt`, or a `captain_event` / `player_event` carrying a non-`text_delta` (tool-lifecycle) event — it shall issue exactly one `#{pane_in_mode}`-gated copy-mode exit (`send-keys -X cancel`) against that record's destination pane.
When it observes a record that produces no pane write — a `turn_started`, `turn_finished`, `captain_prompt`, or `captain_telemetry` control record, or a `captain_event` / `player_event` carrying a buffered `text_delta` event that only accumulates into the open block — it shall issue no copy-mode exit for that record; the assertion shall specifically pin that a terminal flush record (`captain_finished` / `player_finished`) does trigger the gated exit so the final buffered block is followed even when a scroll happens after the last delta but before the flush.
The gated exit shall target the player pane for a record carrying a `playerId` and the Boss/Captain pane (pane index 0) for `captain_*` / `turn_aborted` / `runtime_error` records per [TMUX-040](../user/tmux-play.md#tmux-040), rapid write-driving records to the same pane within the debounce window shall coalesce to a single exit, and an exit failure shall be swallowed (display-only) without aborting the turn.
Given a real tmux server, when a pane in the launched session is seeded with scrollback and scrolled back so that `#{scroll_position}` is greater than `0` and `#{pane_in_mode}` is `1`, and new content is then written to that pane through a block flush, the pane shall return to its live tail with `#{pane_in_mode}` reporting `0` and the new content visible; a sibling pane in the session that receives no concurrent write and is scrolled back shall keep its `#{scroll_position}` and remain at `#{pane_in_mode}` `1`.
The probe shall assert the pane's pre-write `#{scroll_position}` is greater than `0` so a setup that failed to scroll back fails loudly rather than passing vacuously, and shall not require adapter API keys (it may seed deterministic history, for example via `respawn-pane`, since the follow is `pane_in_mode`-gated and independent of pane contents).
