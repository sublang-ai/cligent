...
Given a real tmux server running session mode in a Boss/Captain pane of width `W` whose visible region is first filled so the `boss> ` prompt sits on the pane's bottom row (for example by submitting enough empty lines to scroll earlier prompt rows into history), when the Boss types and edits — character by character, including edits around each `W`-column boundary — an input line whose prompt-plus-content visible width reaches at least `2·W` and whose every wrapped display row carries distinct content, then capturing the pane and its history with `tmux capture-pane -p -S -<n>` and stripping ANSI bytes shall yield, among the rows that carry the active input's typed content, no row value appearing more than once (no input row is duplicated into scrollback) and no row of visible width `W` cells (the rightmost column stays unused).
The empty prompt-only `boss> ` rows left by the prefill are intentionally repeated and are excluded from this no-duplication check; the distinct per-row content of the active input makes its rows identifiable apart from those prefill rows.
The active input's prompt-stripped rows, concatenated in order, shall equal the typed text, so the wrap-correct renderer drops and duplicates no characters.
The probe shall submit no Boss turn (so it needs no adapter API key), shall run under `*.acceptance.test.ts`, and shall self-skip when either `tmux -V` or `glow -v` fails; the ESC-interrupt and bracketed-paste continuity that [TMUX-067](../user/tmux-play.md#tmux-067) carries forward from [TMUX-057](../user/tmux-play.md#tmux-057) / [TMUX-058](../user/tmux-play.md#tmux-058) is verified at the session level by [TTMUX-059](#ttmux-059) and [TTMUX-060](#ttmux-060).
