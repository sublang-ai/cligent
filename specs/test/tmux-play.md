...
Given the built-in fanout Captain and the four API-key-runnable adapters (`claude`, `codex`, `gemini`, and `opencode`) as players with valid credentials, when handling a Boss turn that requires a sentinel token in every reply, every `player_finished` shall report `status: 'ok'` with the sentinel in `finalText`, the single `captain_prompt` shall contain one delimited result section per player that names its status and includes its final text with the sentinel, and `captain_finished` shall report `status: 'ok'` with the sentinel in `finalText`. `runtime_error` and `turn_aborted` shall not appear.
When `CLIGENT_KIMI_ACCEPTANCE_HOME` explicitly names an absolute, dedicated Kimi home authenticated by `kimi login`, the same run shall additionally include the `kimi` player while dereferencing and cloning only that source home's configuration and OAuth credentials into a permission-hardened temporary `KIMI_CODE_HOME`. Without that explicit home, the fanout run shall log that the Kimi leg was omitted; exact Kimi ACP initialization and runtime registration remain covered by package conformance and adapter/runtime tests. The source fixture shall be treated as disposable because an OAuth refresh against the clone may leave its prior token stale.
