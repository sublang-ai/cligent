...
Given the launcher constructing a tmux-play session, the tmux command stream shall include `set-option -t <session> mouse on`, shall not include a `set-clipboard` option write, and shall not include any `bind-key` command for `Mouse*` or `Wheel*` events.
Given a real tmux server, when `launchTmuxPlay({ attach: false })` returns, `tmux show-options -v -t <session> mouse` shall report `on`.
