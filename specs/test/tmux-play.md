...
_Status: retired and entirely non-normative. The paragraphs below record the original verification criteria in past tense for spec history; no clause in this item is in effect, and no `shall` text appears here. The active verification of left-click behavior in a tmux-play session is owned by [TTMUX-067](#ttmux-067), which pins the inverse contract: the launcher installs only tmux's stock per-table `MouseDown1Pane` bindings, and no `MouseDown1Pane` argv references the launched session name, `pane_in_mode`, or `-X cancel`._
