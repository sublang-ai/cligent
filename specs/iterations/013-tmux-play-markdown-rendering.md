...
- Speaker-prefix post-indent so [[tmux-play-38](../packages/tmux-play.md#tmux-play-38)] grammar holds; the prefix budget keeps prose rows within pane width.
...
- New tmux-play-50 (pipeline, prefix-budgeted width, prefix preservation) and tmux-play-51 (`glow` launch gate). Amendments to tmux-play-38 (prefix on rendered block), tmux-play-46 (soft-wrap superseded; cell measurement retained for [[tmux-play-49](../packages/tmux-play.md#tmux-play-49)]), and tmux-play-49 (tool body through the pipeline).
- Matching TTMUX items and a real-`glow` acceptance probe. The `Real-tmux Acceptance` preamble and tmux-play-139's standalone clause both broadened to self-skip when `glow -v` fails, since the launcher hard-fails otherwise.
...
- [x] `src/app/tmux-play/presenter-tmux.ts` — buffer-then-render path; removal of tmux-play-46 soft-wrap machinery.
...
- [x] `specs/user/tmux-play.md` — new tmux-play-50 and tmux-play-51; amendments to tmux-play-38, tmux-play-46, tmux-play-49.
- [x] `specs/test/tmux-play.md` — new TTMUX items; `Real-tmux Acceptance` preamble and tmux-play-139 broadened to self-skip on missing `glow`.
...
1. [x] **Render module and launch gate** — `isGlowAvailable` + `renderMarkdown(text, width)` in `shared/glow.ts`; launcher `glow` gate. New tmux-play-51.
2. [x] **Buffer-then-render presenter** — buffer text deltas, flush on the boundaries in Scope, render at `paneWidth - prefixWidth`, post-indent. Remove tmux-play-46 soft-wrap machinery. New tmux-play-50; amend tmux-play-38 and tmux-play-46.
3. [x] **Tool-result body through the pipeline** — fence the body with `max(3, longest_backtick_run + 1)` backticks, render, indent. Amend tmux-play-49.
