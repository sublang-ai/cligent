...
- [ ] Canonical Kimi, engine, tmux-play, package, and acceptance items define the supported boundary.
- [ ] `KimiAdapter` drives one `kimi acp` process per run and normalizes text, tools, permissions, terminal results, and failures.
- [ ] Fresh and resumed sessions, abort cancellation, and resume tokens preserve continuity.
- [ ] Kimi is available through the public package, effort metadata, tmux-play configuration, and runtime adapter registry.
- [ ] Exact ACP SDK and Kimi CLI conformance targets are installed and verified without adding a Kimi-specific SDK peer.
- [ ] Unit, package, smoke, and credential-gated live acceptance checks pass.
- [ ] User documentation and the unreleased changelog describe setup, authentication, capabilities, and intentional limitations.

## Tasks

Each task is one-commit size and keeps build, typecheck, lint, and focused tests green at its boundary.

1. [ ] **Specify the Kimi integration.**
...
3. [ ] **Register Kimi across public surfaces.**
...
4. [ ] **Verify the distributable and live target.**
...
5. [ ] **Document and verify the feature.**
...
- Unsupported bypass, per-capability/no-mode policies, explicit tool lists, turn/budget limits, and non-Kimi effort values fail before the Kimi process starts.
