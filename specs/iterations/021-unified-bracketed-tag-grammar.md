...
Why the call glyph is `↪`:
`↪` (U+21AA, Rightwards Arrow With Hook) reads as entering the tool invocation without introducing the heavier branch-only feel of the retired `⤷`.
...
The call arrow is 1-cell narrow per [TMUX-046](../user/tmux-play.md#tmux-046)'s cell-measurement rules.
