...
- [[tmux-play-6](../packages/tmux-play.md#tmux-play-6)], [[tmux-play-7](../packages/tmux-play.md#tmux-play-7)], [[tmux-play-29](../packages/tmux-play.md#tmux-play-29)], `CaptainConfig`, `PlayerConfig`, `RuntimeCaptainConfig`, `RuntimePlayerConfig`, and `CreatePlayerCligentOptions`: add `reasoningEffort?: ReasoningEffort` (the [[engine-20](../packages/engine.md#engine-20)] closed set), forwarded into each constructed `Cligent` as `CligentOptions.reasoningEffort` per [[engine-1](../packages/engine.md#engine-1)].
- YAML loader: validate `reasoningEffort` against the [[engine-20](../packages/engine.md#engine-20)] closed set; reject out-of-set values at launcher startup with an error naming the offending path (e.g. `captain.reasoningEffort`, `players[0].reasoningEffort`), per [[tmux-play-8](../packages/tmux-play.md#tmux-play-8)] and [DR-005](../decisions/005-per-adapter-permission-configuration.md)'s failure-surfacing rule.
...
Gemini 3 exposes four rungs (`MINIMAL` / `LOW` / `MEDIUM` / `HIGH`); `xhigh` and `max` collapse to `HIGH` per the [[engine-20](../packages/engine.md#engine-20)] "nearest supported neighbour" rule.
...
`max` maps to the model's documented upper bound rather than Google's `-1` dynamic-thinking sentinel: per [[engine-20](../packages/engine.md#engine-20)] `max` is "the greatest reasoning depth," which is the upper bound, not adaptive.
On Flash and Flash Lite, `xhigh` and `max` collapse to `24576` (the per-model ceiling) per the [[engine-20](../packages/engine.md#engine-20)] nearest-neighbour rule.
...
- A YAML config whose `captain.reasoningEffort` or `players[i].reasoningEffort` is in the [[engine-20](../packages/engine.md#engine-20)] closed set loads and forwards the value to that player's or the captain's `Cligent` as `CligentOptions.reasoningEffort`.
