...
- [TMUX-006](../user/tmux-play.md#tmux-006), [TMUX-007](../user/tmux-play.md#tmux-007), [TMUX-029](../user/tmux-play.md#tmux-029), `CaptainConfig`, `PlayerConfig`, `RuntimeCaptainConfig`, `RuntimePlayerConfig`, and `CreatePlayerCligentOptions`: add `reasoningEffort?: ReasoningEffort` (the [[engine-20](../packages/engine.md#engine-20)] closed set), forwarded into each constructed `Cligent` as `CligentOptions.reasoningEffort` per [[engine-1](../packages/engine.md#engine-1)].
- YAML loader: validate `reasoningEffort` against the [[engine-20](../packages/engine.md#engine-20)] closed set; reject out-of-set values at launcher startup with an error naming the offending path (e.g. `captain.reasoningEffort`, `players[0].reasoningEffort`), per [TMUX-008](../user/tmux-play.md#tmux-008) and [DR-005](../decisions/005-per-adapter-permission-configuration.md)'s failure-surfacing rule.
...
- [x] `specs/user/tmux-play.md` — TMUX-006, TMUX-007, and TMUX-029 extended to admit `reasoningEffort?` on players and captain.
