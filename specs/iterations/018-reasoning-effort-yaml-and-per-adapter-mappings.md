...
- [x] `specs/user/tmux-play.md` — TMUX-006, TMUX-007, and TMUX-029 extended to admit `reasoningEffort?` on roles and captain.
...
- [x] `src/app/tmux-play/roles.ts` — `RoleConfig` and `CreateRoleCligentOptions` widened; role construction forwards `reasoningEffort` to the constructed `Cligent`.
- [x] `src/app/tmux-play/contract.ts` — `RuntimeCaptainConfig` and `RuntimeRoleConfig` widened.
