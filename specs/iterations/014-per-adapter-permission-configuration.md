...
- [x] `src/app/tmux-play/config.ts` — YAML loader accepts and validates `permissions` on captain and players per [TMUX-052](../user/tmux-play.md#tmux-052).
...
4. [x] **YAML schema + loader + session wiring** — extend `PlayerConfig` and the captain config with `permissions?: PermissionPolicy`; YAML loader accepts the typed shape and rejects malformed sub-fields per TMUX-008; `session.ts` forwards the value into `CligentOptions.permissions` at player / captain construction. New TMUX items for the YAML field; loader tests for accept + reject.
