...
- Add `yaml` runtime dep; amend [PKG-003](../dev/package.md#pkg-003).
- Drop dead JS-config plumbing (`defineConfig`, JS-path rewriting in the snapshot).

Out of scope: see [DR-004 §Out of Scope](../decisions/004-tmux-play-captain-architecture.md#out-of-scope).

## Deliverables

- [ ] `specs/dev/package.md` — amend PKG-003 to permit the `yaml` runtime dep.
- [ ] `package.json` — add `yaml` to `dependencies`.
- [ ] `src/app/tmux-play/config.ts` — YAML loader, cwd→home discovery, first-run auto-create with stdout notice.
- [ ] `src/app/tmux-play/index.ts` — drop `defineConfig`.
- [ ] Tests: loader round-trips YAML; cwd wins over home; auto-create writes the default; existing home file is preserved across runs; malformed YAML hard-errors with file context.
...
1. Amend PKG-003 in `specs/dev/package.md` to permit the `yaml` runtime dep.
2. Replace JS/JSON config discovery with the YAML loader (cwd + home fallback + first-run auto-create); add `yaml` dep; drop `defineConfig` and JS-path rewriting; tests included.
3. Update README and `docs/tmux-play.md` for the YAML/auto-create flow.
