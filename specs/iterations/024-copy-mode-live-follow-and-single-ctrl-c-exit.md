...
- New [TTMUX-069](../test/tmux-play.md#ttmux-069): a unit-level assertion that the live-follow observer issues a `#{pane_in_mode}`-gated copy-mode-exit for the destination pane on output-bearing records and not on non-output records, plus a real-tmux acceptance probe that seeds scrollback into a pane, scrolls it back (`#{scroll_position} > 0`, `#{pane_in_mode} = 1`), drives new content to that pane, and asserts the pane returns to its live tail (`#{pane_in_mode} = 0`) with the new content visible.
...
Empirically verified against tmux 3.6b:
