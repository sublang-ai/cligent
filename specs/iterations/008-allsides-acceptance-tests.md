...
Add end-to-end acceptance tests that exercise the full AllSides pipeline (prompt → tool use → response → done) with real API keys while minimizing token spend.
...
`apps/allsides/src/acceptance.test.ts`

- Creates a temp work dir with log files and a `.allsides-session` marker
- Calls `resolveAgents()` with all four agents explicitly
- Runs session orchestration with prompt `"List the files in the current directory"`
- Reads each agent's log file after completion
- Asserts each log contains: boss echo, at least one text output, at least one `[tool: ...]` line, and a `[success | ...]` done line
...
- [ ] `apps/allsides/src/acceptance.test.ts` — acceptance test file
...
2. **Write acceptance test** (`apps/allsides/src/acceptance.test.ts`)
   - Create temp work dir via `mkdtempSync`
   - Create empty `<agent>.log` files and `.allsides-session` marker
   - Call `resolveAgents()` with explicit entries for all four agents
   - For each agent, run `cligent.run("List the files in the current directory", { abortSignal })` and drain events to the log file using `formatEvent()`
   - Read each agent's log after completion
   - Assert: boss echo present, at least one text output line, at least one `[tool: ...]` line, `[success | ...]` done line
