...
| test | [tmux-play.md](test/tmux-play.md) | tmux-play config discovery, runtime causality, observer dispatch, topology, fanout acceptance, permission / reasoning-effort / layout configuration, run-time timers, Boss input keybindings, mouse focus-without-deselect (no `MouseDown1Pane` override; tmux stock bindings preserve scroll position and selection across focus changes), session-wide Ctrl+C exit, real-tmux acceptance, real-glow acceptance |
