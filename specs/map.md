...
| Group | File | Summary |
| --- | --- | --- |
| user | [tmux-play.md](user/tmux-play.md) | tmux-play CLI, YAML config (top-level `theme: mocha \| latte \| auto`, top-level `layout` with `window` resolution and `columnWeights`, per-player/captain `permissions` and `reasoningEffort`), discovery and auto-create, Captain contract, records, observers, topology, mouse selection + system clipboard right-click copy + left-click behavior that drops any active selection in every pane while preserving each pane's copy-mode state and scroll position, OSC 11 flavor-aware Catppuccin theme + diagnostics, run-time timers, Boss input keybindings, session-wide Ctrl+C exit, speaker colors, tool lifecycle, glow-rendered Markdown pipeline + launcher gate, runtime API, fanout |
