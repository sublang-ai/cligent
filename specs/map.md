...
| Group | File | Summary |
| --- | --- | --- |
| user | [adapters/opencode.md](user/adapters/opencode.md) | OpenCode adapter: SSE normalization, `callID`-correlated tool lifecycle, wildcard auto permissions with deterministic headless replies, two modes, current-session filtering, finite relevant-event inactivity with status recovery and cleanup, resume token, options mapping, portable-effort variants |
