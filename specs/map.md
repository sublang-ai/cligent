...
| Group | File | Summary |
| --- | --- | --- |
| user | [tmux-play.md](user/tmux-play.md) | tmux-play CLI, YAML config (top-level `theme: mocha \| latte \| auto`, per-player/captain `permissions` and `reasoningEffort`), discovery and auto-create, Captain contract, records, observers, topology, mouse selection, OSC 11 flavor-aware Catppuccin theme + diagnostics, run-time timers, Boss input keybindings, speaker colors, tool lifecycle, glow-rendered Markdown pipeline + launcher gate, runtime API, fanout |
