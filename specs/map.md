...
| Group | File | Summary |
| --- | --- | --- |
| user | [tmux-play.md](user/tmux-play.md) | tmux-play CLI, YAML config (top-level `theme: mocha \| latte \| auto`, top-level `layout` with `window` resolution and `columnWeights`, per-player/captain `permissions` and `reasoningEffort`), discovery and auto-create, Captain contract, records, observers, topology, mouse selection + system clipboard right-click copy + focus-only left-click (scroll position and any active copy-mode selection preserved across focus changes), OSC 11 flavor-aware Catppuccin theme + diagnostics, run-time timers, Boss input keybindings, session-wide Ctrl+C exit, speaker colors, tool lifecycle, glow-rendered Markdown pipeline + launcher gate, runtime API, fanout |
| test | [tmux-play.md](test/tmux-play.md) | tmux-play config discovery, runtime causality, observer dispatch, topology, fanout acceptance, permission / reasoning-effort / layout configuration, run-time timers, Boss input keybindings, stock `MouseDown1Pane` bindings (no cancel-on-left-click chain, no session-scoped `if-shell` override; the explicit stock re-install clears any stale TMUX-066 entry on a server-global key table and preserves scroll position and any active copy-mode selection across focus changes), session-wide Ctrl+C exit, real-tmux acceptance, real-glow acceptance |
