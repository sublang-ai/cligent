...
- on the environment-overlay route the harness shall require only the CLI, clone no fixture, and run no usability probe, an overlay-supplied key carrying no rotation state that could spend it between runs;
- under `CI` the environment-overlay route shall be configured, so every live Kimi leg exercises the adapter rather than self-skipping, and its absence shall leave the OAuth route's outcomes unchanged;
