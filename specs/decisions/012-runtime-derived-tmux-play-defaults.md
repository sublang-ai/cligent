...
- Repair commands are scoped to the tree the running package resolves from: a peer SDK follows cligent's own install scope [[package-32](../packages/package.md#package-32)], an external CLI is always global, and the reported tree and command follow the launcher repair contract [[tmux-play-89](../packages/tmux-play.md#tmux-play-89)] so a layout no canned command repairs stays diagnosable.
  ...
  And every peer-SDK command installs [[package-16](../packages/package.md#package-16)]'s exact installation-repair package specifier and names the tree with `--prefix`, because npm's command line outranks both its environment and its project discovery, so the pinned form lands in the named tree in every context where the bare form would only sometimes.
  ...
  External CLI commands install [[package-16](../packages/package.md#package-16)]'s exact installation-repair package specifier globally but carry no `--prefix`: the executable is found through `PATH`, so it belongs in whatever global prefix the user's shell already reads.
